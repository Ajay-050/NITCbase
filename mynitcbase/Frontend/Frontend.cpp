...
int Frontend::select_attrlist_from_table_where(char relname_source[ATTR_SIZE], char relname_target[ATTR_SIZE],
  ...
  char target_rel[ATTR_SIZE];
  strcpy(target_rel,TEMP);
  int ret = Algebra::select(relname_source,target_rel,attribute,op,value);
  if(ret!=SUCCESS){
     return ret;
  }
  int id =OpenRelTable::openRel(target_rel);
  if(id<0){
    Schema::deleteRel(target_rel);
    return id;
  }
  int ret1  = Algebra::project(target_rel,relname_target,attr_count,attr_list);
  Schema::closeRel(target_rel);
  //printf("hi");
  Schema::deleteRel(target_rel);
  if(ret1!=SUCCESS){
    return ret1;
